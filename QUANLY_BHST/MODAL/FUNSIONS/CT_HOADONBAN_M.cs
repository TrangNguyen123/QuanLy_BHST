   ...
   public class CT_HOADONBAN_M
    {
        ...
        public DataTable Get_Obj()
        {
            ...
            //lấy tên thủ tục
            cmd.CommandText = "loatcthoadonban";
        ...
        public bool Add_Obj(CHITIET_HDB obj)
        {
            try
            {
                conn.OpenConn();
                //    DataTable dt = new DataTable();
                SqlCommand cmd = new SqlCommand("themcthoadonban", conn.SQL_CONN);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public bool Up_Obj(CHITIET_HDB obj)
        {
            try
            {
                conn.OpenConn();
                //    DataTable dt = new DataTable();
                SqlCommand cmd = new SqlCommand("suacthoadonban", conn.SQL_CONN);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.Add(new SqlParameter("@mahoadon", obj.Mahoadon));
                cmd.Parameters.Add(new SqlParameter("@mavach", obj.Mavach));
                cmd.Parameters.AddWithValue("@soluong", Convert.ToInt32(obj.Soluong));
        ...
        public bool Del_Obj(string obj, string obj1)
        {
            try
            {
                conn.OpenConn();
                //    DataTable dt = new DataTable();
                SqlCommand cmd = new SqlCommand("xoacthoadonban", conn.SQL_CONN);
                cmd.CommandType = CommandType.StoredProcedure;
